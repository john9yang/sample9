...
public class GridCollector {
    ...
    private GridCollector( GridSpec gridSpec ){
        ...
        IntStream.range(0,spec.rows*spec.cols).forEach(i -> cells.add(new ArrayList<>()));
    }
    ...
    public GridCollector merge( GridCollector other ){
        //cell-wise merge
        IntStream.range(0,cells.size()).forEach(i -> cells.get(i).addAll(other.cells.get(i)));
        return this;
    }
    ...
    public static Collector<Box,?,List<List<Box>>> of( GridSpec gridSpec ){
        return Collector.<Box,GridCollector,List<List<Box>>>of(
                () -> new GridCollector(gridSpec),
                GridCollector::update,
                GridCollector::merge,
                GridCollector::finish,
